...
struct NonStreamRequestContext<'a> {
    ...
    cache_tracker: Option<&'a std::sync::Arc<crate::anthropic::cache_tracker::CacheTracker>>,
    cache_profile: Option<&'a crate::anthropic::cache_tracker::CacheProfile>,
    /// 是否开启非流式响应的 thinking 块提取（由全局配置 extract_thinking 且请求启用 thinking 决定）
...
async fn handle_non_stream_request(
    ...
    // 直接用 reasoning_text。
    //
    // 仅在全局配置 extract_thinking 开启时才做提取/拆分；否则原样返回 assistant 文本，
    // 保持与旧版行为一致（避免默认改变非流式响应结构）。
    let (final_thinking, final_text) = if context.extract_thinking {
        ...
        (final_thinking, cleaned_text)
    } else {
        // 未开启提取：thinking 仅取自独立 reasoning 流（若有），文本保持原样
        (reasoning_text, text_content)
    };
