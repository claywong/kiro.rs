...
/// 对 API Key 做脱敏展示：保留前 4 位与后 4 位，中间以 …… 代替。
...
impl MultiTokenManager {
    ...
    pub fn new(
        ...
        for entry in &mut entries {
                    ...
                    .unwrap_or(false)
            {
                ...
                entry.disabled = true;
                entry.disable_reason = Some(DisableReason::InvalidConfig);
            }
        }
    ...
    /// 获取当前负载均衡模式（"priority" 或 "balanced"）
    pub fn get_load_balancing_mode(&self) -> String {
        self.config.read().load_balancing_mode.clone()
    }
        ...
        ManagerSnapshot {
            entries: entries
                ...
                .map(|e| {
                    ...
                    // 仅 API Key 凭据计算 apiKeyHash / maskedApiKey，用于前端去重与展示
                    let api_key_hash = e
                        .credentials
                        .kiro_api_key
                        .as_deref()
