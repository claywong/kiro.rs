...
| `stockPollIntervalSecs` | 无 webhook（或卖家已停推）时的轮询间隔，0 = 关闭 | `0` |
| `stockPollEnabled` | 轮询开关，面板可切；不写时按间隔是否非 0 推导 | 随间隔 |
...
`legacy`、`kiroapp`（.io）、`drop`、`kiromarket` 与 `kiro-ooo` 协议上支持推送；
`kiroapp-cc` 与 `kirored` 没有 webhook —— 这两家的自动提取要靠
[库存轮询](#库存轮询没有-webhook-的家怎么自动提取)，光开 `autoPurchase` 不会有任何动作。

**协议支持 ≠ 卖家在推。** kiro.ceo（`legacy`）已停止推送，虽然入站端点仍可用，它的
自动提取现在同样依赖库存轮询。判断某家到底还有没有触发源，看面板事件列表最近有没有
卖家推来的事件（轮询合成的那些 id 带 `poll:` 前缀，一眼可分）。
...
**卖家停推之后同理。** kiro.ceo（`legacy`）原本推 webhook，现已不推，于是它也落到
上面那种状态里 —— 入站端点还在、`autoPurchase` 还开着，但没人来叫醒。该家现在靠
轮询作为唯一触发源（配置见下）。入站端点保留不占成本，卖家哪天恢复推送就自动是
更及时的那条路，两者并存不会重复下单。
...
| 字段 | 含义 |
|---|---|
| `stockPollIntervalSecs` | 轮询间隔秒数，**0 = 关闭（默认）**。有下限，配小了抬到下限并告警 |
| `stockPollEnabled` | 轮询开关，面板可切。不写时按 `stockPollIntervalSecs > 0` 推导 |
| `stockPollRespectGlobalGate` | 是否遵循全局总闸 `autoPurchaseEnabled`，默认 `true` |

### 间隔下限按家分档

查一次库存的代价各家差很多，故下限不是一个数：

| 家 | 下限 | 为什么 |
|---|---|---|
| `legacy`（kiro.ceo） | **5 秒** | 查库存就是一次带 `X-API-Key` 的裸 `GET /api/my/stock`，没有登录、签名、解密 |
| 其余（含 `kirored`） | 60 秒 | kiro.red 一轮要登录换 JWT + 请求签名 + 响应解密；且它们的车次以十分钟计，1 分钟分辨率本就够 |

放宽 legacy 是因为它现在**没有 webhook 兜底**，轮询的分辨率直接决定抢不抢得到 ——
60 秒的下限会让先到的人把货买空。kiro.ceo 线上配的是 10 秒。
...
| | 管什么 | 改了要重启吗 |
|---|---|---|
| `stockPollIntervalSecs` | 节奏（多久查一次） | 要 |
| `stockPollEnabled` | 开不开 | 不要，面板随时切 |

早先只有间隔一个旋钮，「停掉轮询」得把它改成 0，而那会让轮询器根本不起、改回来
必须重启 —— 面板上既停不了也恢复不了。`stockPollEnabled` 补上这一环：轮询器只要
间隔非 0 就起，每轮读开关决定要不要真查库存。关掉后它与手动提取下的待机同一形态
（整轮跳过、连库存都不查），打开后最迟一个周期恢复。

`stockPollEnabled` 管的**只是现货这一支**，两条路不受它影响：

- 卖家 webhook 推送触发的自动提取
- kiro.red 自动预定（它排在开关判定之前，由 `autoReserve` 单独管）

想整家停掉自动扣费仍是关 `autoPurchase`（现货）与 `autoReserve`（预定）。

显式写 `"stockPollEnabled": true` 但没配间隔时，按该家下限跑而不是当成关闭 ——
否则那会是个点得动、也写回了文件、却永远不动的开关。

### 「发现到货」的判据分两种

合成事件的 id 必须只由**批次身份**决定 —— 取时间戳会让每轮产生一条新事件（等于每
个周期撞一次授权判定），取固定串则第一次之后永远算重投。批次身份从哪来，按卖家
给不给发车时刻分两条：

| 卖家 | 判据 | 批次身份 |
|---|---|---|
| 给发车时刻（`kirored` / `kiroooo` 等） | 有开放且有货的区 | 区 + 发车时刻 |
| 不给（`legacy`，即 kiro.ceo） | 该区库存**从 0 变正**的边沿 | 区 + 发现时刻 |

kiro.ceo 的库存是「某区当前还剩几个」这种连续量，卖家压根不给发车时刻，故第一条
判据对它永远不成立 —— 只有它的话，给该家配上轮询会是静默无效的。边沿是这里能取到
的唯一批次身份，也正是 webhook 时代 `new_keys_available` 的语义：**补货发生了一次**。

两个代价要清楚：

- **库存持续为正时不会再触发。** 卖家常备库存不清零的话，只有进程启动后的第一次
  补货能被发现。刻意的保守取向 —— 宁可漏一趟，不可每轮下单。真需要在有货时补，
  那条路是 webhook 或人工在面板上提。
- **进程重启、以及关掉再打开开关，都会重建基线，首轮只记录不触发。** 此刻分不出
  「刚补的货」与「早就摆在那儿的存货」，把后者当到货会让每次重启都下一单。

基线放在进程内存里，不落库 —— 落库反而会让「重启前摆着的存货」在重启后被当成新到货。
