...
import { Activity, AlertTriangle, BellRing, Calendar, Coins, Cpu, KeyRound, Server } from 'lucide-react'
import { useByCredential, useByKey, useByModel, useOverview, useRecentHealth, useTimeSeries } from '@/hooks/use-stats'
...
function StatsCards({ stats, timeText }: { stats: RangeStats; timeText: string }) {
  const cards = [
    {
      icon: <Activity className="h-4 w-4" />,
      label: '调用',
